...
class_list = np.loadtxt('class.names',dtype='str',delimiter='\n')
...
def draw_box(image, result_confidences, result_class_ids, result_boxes):
    ...
    for i in range(len(result_class_ids)):
        ...
        label = "{} {}".format(class_list[class_id], round(conf*100,2))
        # print(label, box)
...
while True:
    ...
    fps = "{} FPS".format(int(fps))

    cv2.putText(image_output, fps, (7, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (100, 255, 0), 2, cv2.LINE_AA)
